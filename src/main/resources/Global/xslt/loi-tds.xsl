...
-->
	...
	<xsl:template match="PID">
		<xsl:param name="vertical-orientation" as="xs:boolean"/>
		<xsl:param name="counter"/>
		<xsl:value-of select="util:title('title', concat('Patient Information', $counter), concat('Patient Information', $counter), $ind1, false(), $vertical-orientation, false())"/>
		...
		<xsl:value-of select="util:element('Date/Time of Birth', util:format-time(.//PID.7.1), $ind1)"/>
		...
		<xsl:for-each select=".//PID.13">
			<xsl:choose>
				<xsl:when test=".//PID.13.3 = 'PH'">
					<xsl:value-of select="util:element('Home phone number', util:format-tel2(.//PID.13.6, .//PID.13.7, .//PID.13.8), $ind1)"/>
				</xsl:when>
		...
		<xsl:for-each select=".//PID.14">
			<xsl:choose>
				<xsl:when test=".//PID.14.3 = 'PH'">
					<xsl:value-of select="util:element('Business phone number', util:format-tel2(.//PID.14.6, .//PID.14.7, .//PID.14.8), $ind1)"/>
				</xsl:when>
				<xsl:when test=".//PID.14.3 = 'X.400' or .//PID.14.3 = 'Internet'">
					<xsl:value-of select="util:element('Business email address', .//PID.14.4, $ind1)"/>
				</xsl:when>
			</xsl:choose>
		</xsl:for-each>
	...
	<xsl:template match="NK1">
		<xsl:param name="vertical-orientation" as="xs:boolean"/>
		<xsl:param name="counter"/>
		<xsl:value-of select="util:title('title', concat('Next of kin information', $counter), concat('Next of kin information', $counter), $ind1, true(), $vertical-orientation, false())"/>
		<xsl:value-of select="util:elements($ind1)"/>
		...
		<xsl:for-each select=".//NK1.5">
			<xsl:choose>
				<xsl:when test=".//NK1.5.3 = 'PH'">
					<xsl:value-of select="util:element('Phone number', util:format-tel2(.//NK1.5.6, .//NK1.5.7, .//NK1.5.8), $ind1)"/>
				</xsl:when>
		...
		<xsl:value-of select="util:chooseAmongThree('Contact role', .//NK1.7.9, .//NK1.7.2, .//NK1.7.1, $ind1)"/>
	...
	<xsl:template match="PV1">
		<xsl:param name="vertical-orientation" as="xs:boolean"/>
		<xsl:param name="counter"/>
		<xsl:value-of select="util:title('title', concat('Visit Information', $counter), concat('Visit Information', $counter), $ind1, false(), $vertical-orientation, false())"/>
		<xsl:value-of select="util:elements($ind1)"/>
	...
	<xsl:template match="IN1">
		...
		<xsl:value-of select="util:element('Insured''s Date Of Birth', util:format-time(.//IN1.18.1), $ind1)"/>
	...
	<xsl:template match="GT1">
		<xsl:param name="vertical-orientation" as="xs:boolean"/>
		<xsl:param name="counter"/>
		<xsl:value-of select="util:title('title', concat('Guarantor Information', $counter), 'Guarantor Information', $ind1, false(), $vertical-orientation, false())"/>
		<xsl:value-of select="util:elements($ind1)"/>
		<xsl:value-of select="util:element('Guarantor Name', concat(util:format-with-space(.//GT1.3.2), util:format-with-space(.//GT1.3.3), util:format-with-space(.//GT1.3.1.1), .//GT1.3.4), $ind1)"/>
		<xsl:value-of select="util:format-address-multilines('Guarantor Address', .//GT1.5.1.1, .//GT1.5.2, concat(util:format-with-space(.//GT1.5.3), util:format-with-space(.//GT1.5.4), util:format-with-space(.//GT1.5.5)), .//GT1.5.6, $ind1)"/>
		<xsl:for-each select=".//GT1.11">
			<xsl:choose>
				<xsl:when test="count(.//GT1.11.9) &gt; 0">
					<xsl:value-of select="util:element('Guarantor Relationship', .//GT1.11.9, $ind1)"/>
				</xsl:when>
				<xsl:when test="count(.//GT1.11.9) = 0 and count(.//GT1.11.2) &gt; 0">
					<xsl:value-of select="util:element('Guarantor Relationship', .//GT1.11.2, $ind1)"/>
				</xsl:when>
				<xsl:when test="count(.//GT1.11.9) = 0 and count(.//GT1.11.2) = 0 and count(.//GT1.11.1) &gt; 0">
	...
	<xsl:template match="ORC">
		<xsl:param name="vertical-orientation" as="xs:boolean"/>
		<xsl:param name="counter"/>
		<xsl:value-of select="util:title('title', concat('Order', $counter), 'Order', $ind1, true(), $vertical-orientation, false())"/>
		...
		<xsl:value-of select="util:element('Provider Name', concat(util:format-with-space(.//ORC.12.3), .//ORC.12.2), $ind1)"/>
		...
		<xsl:for-each select=".//ORC.14">
			<xsl:choose>
				<xsl:when test=".//ORC.14.3 = 'PH'">
					<xsl:value-of select="util:element('Call Back Phone number', util:format-tel2(.//ORC.14.6, .//ORC.14.7, .//ORC.14.8), $ind1)"/>
				</xsl:when>
		...
		<xsl:choose>
			<xsl:when test="count(.//ORC.22)">
				<xsl:value-of select="util:format-address-multilines('Ordering Facility Address', .//ORC.22.1.1, .//ORC.22.2, concat(util:format-with-space(.//ORC.22.3), util:format-with-space(.//ORC.22.4), util:format-with-space(.//ORC.22.5)), .//ORC.22.6, $ind1)"/>
			</xsl:when>
		</xsl:choose>
		<xsl:for-each select=".//ORC.23">
			<xsl:choose>
				<xsl:when test=".//ORC.23.3 = 'PH'">
					<xsl:value-of select="util:element('Ordering Facility Phone number', util:format-tel2(.//ORC.23.6, .//ORC.23.7, .//ORC.23.8), $ind1)"/>
				</xsl:when>
				<xsl:when test=".//ORC.23.3 = 'X.400' or .//ORC.23.3 = 'Internet'">
					<xsl:value-of select="util:element('Ordering Facility email address', .//ORC.23.4, $ind1)"/>
				</xsl:when>
			</xsl:choose>
		</xsl:for-each>
		...
		<xsl:value-of select="util:begin-sub-table($ind2)"/>
		<xsl:value-of select="util:title-no-tab('title', 'General order information', 'General order information', $ind2, false())"/>
		<xsl:value-of select="util:elements($ind2)"/>
		...
		<xsl:choose>
			<xsl:when test="count(.//ORC.20.9) &gt; 0">
				<xsl:value-of select="util:element('Advanced Beneficiary Notice Code', .//ORC.20.9, $ind1)"/>
			</xsl:when>
			<xsl:when test="count(.//ORC.20.9) = 0 and count(.//ORC.20.2) &gt; 0">
				<xsl:value-of select="util:element('Advanced Beneficiary Notice Code', .//ORC.20.2, $ind1)"/>
			</xsl:when>
			<xsl:when test="count(.//ORC.20.9) = 0 and count(.//ORC.20.2) = 0 and count(.//ORC.20.1) &gt; 0">
				<xsl:value-of select="util:element('Advanced Beneficiary Notice Code', .//ORC.20.1, $ind1)"/>
			</xsl:when>
		</xsl:choose>
		...
		<xsl:if test="not(count(..//TQ1) = 0)">
			<xsl:value-of select="util:begin-sub-table($ind2)"/>
			<xsl:value-of select="util:title-no-tab('title', 'Timing/Quantity Information', 'Timing/Quantity Information', $ind2, false())"/>
			<xsl:value-of select="util:elements($ind2)"/>
			...
			<xsl:value-of select="util:chooseAmongThree('Priority', ..//TQ1.9.9, ..//TQ1.9.2, ..//TQ1.9.1, $ind1)"/>
			<xsl:value-of select="util:end-table-fieldset($ind2)"/>
		</xsl:if>
		<!-- Order details subtable -->
		<!--Format time needs to be checked; alternative id too-->
		<xsl:value-of select="util:begin-sub-table($ind2)"/>
		<xsl:value-of select="util:title-no-tab('title', 'Order details', 'Order details', $ind2, false())"/>
		<xsl:value-of select="util:elements($ind2)"/>
		...
		<xsl:value-of select="util:chooseAmongThree('Relevant Clinical Information', ..//OBR.13.9, ..//OBR.13.2, ..//OBR.13.1, $ind1)"/>
		<xsl:value-of select="util:end-table-fieldset($ind1)"/>
		<!-- Notes and comments subtable -->
		<xsl:if test="not(count(..//NTE) = 0)">
			<xsl:value-of select="util:begin-sub-table($ind2)"/>
			<xsl:value-of select="util:title-no-tab('title', 'Notes &amp; comments', 'Notes &amp; comments', $ind1, false())"/>
			<xsl:value-of select="util:elements($ind1)"/>
		...
		<xsl:for-each select="..//PRT">
			...
			<xsl:value-of select="util:begin-sub-table($ind2)"/>
			<xsl:value-of select="util:title-no-tab('title', concat('Result copies', $index), concat('Result copies', $index), $ind2, false())"/>
			<xsl:value-of select="util:elements($ind2)"/>
			<xsl:value-of select="util:element('Instance ID', .//PRT.1.1, $ind1)"/>
			<xsl:value-of select="util:element('NPI Identifier', .//PRT.5.1, $ind1)"/>
			<xsl:value-of select="util:element('Name', concat(util:format-with-space(.//PRT.5.3), .//PRT.5.2), $ind1)"/>
			<xsl:for-each select=".//PRT.14">
				<xsl:value-of select="util:format-address-multilines('Address', ./PRT.14.1/PRT.14.1.1, ./PRT.14.2, concat(util:format-with-space(./PRT.14.3), util:format-with-space(./PRT.14.4), util:format-with-space(./PRT.14.5)), ./PRT.14.6, $ind1)"/>
			</xsl:for-each>
			<xsl:for-each select=".//PRT.15">
				<xsl:choose>
					<xsl:when test=".//PRT.15.3 = 'PH'">
						<xsl:value-of select="util:element('Phone Number', util:format-tel2(.//PRT.15.6, .//PRT.15.7, .//PRT.15.8), $ind1)"/>
					</xsl:when>
			...
			<xsl:value-of select="util:chooseAmongThree('Participation', .//PRT.4.9, .//PRT.4.2, .//PRT.4.1, $ind1)"/>
			<xsl:value-of select="util:end-table-fieldset($ind1)"/>
		</xsl:for-each>
		...
		<xsl:for-each select="..//DG1">
			...
			<xsl:value-of select="util:begin-sub-table($ind2)"/>
			<xsl:value-of select="util:title-no-tab('title', concat('Diagnosis information', $index), concat('Diagnosis information', $index), $ind2, false())"/>
			<xsl:value-of select="util:elements($ind2)"/>
		...
		<xsl:for-each select="..//OBX">
			...
			<xsl:value-of select="util:begin-sub-table($ind2)"/>
			<xsl:value-of select="util:title-no-tab('title', concat('Observation Result - ', .//OBX.1), concat('Observation Result - ', .//OBX.1), $ind2, false())"/>
			<xsl:value-of select="util:elements($ind2)"/>
			<xsl:choose>
				<xsl:when test=".//OBX.3.3 = 'LN'">
					<xsl:value-of select="util:element('Observation Identifier (LOINC code)', .//OBX.3.1, $ind1)"/>
				</xsl:when>
				<xsl:when test=".//OBX.3.6 = 'LN'">
					<xsl:value-of select="util:element('Observation Identifier (LOINC code)', .//OBX.3.4, $ind1)"/>
				</xsl:when>
			</xsl:choose>
			<xsl:choose>
				<xsl:when test="not(.//OBX.3.3 = 'LN')">
					<xsl:value-of select="util:element('Observation Identifier (local code)', .//OBX.3.1, $ind1)"/>
				</xsl:when>
				<xsl:when test="not(.//OBX.3.6 = 'LN')">
					<xsl:value-of select="util:element('Observation Identifier (local code)', .//OBX.3.4, $ind1)"/>
				</xsl:when>
			</xsl:choose>
			<xsl:choose>
				<xsl:when test=".//OBX.2 = 'CWE'">
					<xsl:value-of select="util:chooseAmongThree('Observation Value', .//OBX.5.9, .//OBX.5.2, .//OBX.5.1, $ind1)"/>
				</xsl:when>
				<xsl:when test=".//OBX.2 = 'NM'">
					<xsl:value-of select="util:element('Observation Value', .//OBX.5, $ind1)"/>
				</xsl:when>
				<xsl:when test=".//OBX.2 = 'DT'">
					<xsl:value-of select="util:element('Observation Value', util:format-time(.//OBX.5), $ind1)"/>
				</xsl:when>
			</xsl:choose>
			<xsl:value-of select="util:chooseAmongThree('Units', .//OBX.6.9, .//OBX.6.2, .//OBX.6.1, $ind1)"/>
			<xsl:value-of select="util:element('Observation Result status', .//OBX.11, $ind1)"/>
			<xsl:value-of select="util:element('Date/Time of the Observation', util:format-time(.//OBX.14.1), $ind1)"/>
		...
		<xsl:for-each select="..//SPM">
			...
			<xsl:value-of select="util:begin-sub-table($ind2)"/>
			<xsl:value-of select="util:title-no-tab('title', concat('Specimen Details', $index), concat('Specimen Details', $index), $ind2, false())"/>
			<xsl:value-of select="util:elements($ind2)"/>
			<xsl:value-of select="util:chooseAmongThree('Type', .//SPM.4.9, .//SPM.4.2, .//SPM.4.1, $ind1)"/>
			<xsl:value-of select="util:chooseAmongThree('Type Modifier', .//SPM.5.9, .//SPM.5.2, .//SPM.5.1, $ind1)"/>
			<xsl:value-of select="util:chooseAmongThree('Additives', .//SPM.6.9, .//SPM.6.2, .//SPM.6.1, $ind1)"/>
			<xsl:value-of select="util:chooseAmongThree('Collection Method', .//SPM.7.9, .//SPM.7.2, .//SPM.7.1, $ind1)"/>
			<xsl:value-of select="util:chooseAmongThree('Source site', .//SPM.8.9, .//SPM.8.2, .//SPM.8.1, $ind1)"/>
			<xsl:value-of select="util:chooseAmongThree('Source site modifier', .//SPM.9.9, .//SPM.9.2, .//SPM.9.1, $ind1)"/>
			<xsl:value-of select="util:element('Collection Start Date/Time', util:format-time(.//SPM.17.1.1), $ind1)"/>
			<xsl:value-of select="util:element('Collection End Date/Time', util:format-time(.//SPM.17.2.1), $ind1)"/>
			<xsl:value-of select="util:end-table-fieldset($ind1)"/>
		</xsl:for-each>
		<!--<xsl:value-of select="util:end-table-fieldset($ind1)"/>-->
	...
	<xsl:template match="MSA">
		<xsl:param name="vertical-orientation" as="xs:boolean"/>
		<xsl:value-of select="util:title('title', 'Acknowledgment', 'Acknowledgment', $ind1, false(), $vertical-orientation, false())"/>
		<xsl:value-of select="util:elements($ind1)"/>
	...
	<xsl:template match="ERR">
		<xsl:param name="vertical-orientation" as="xs:boolean"/>
		<xsl:param name="counter"/>
		<xsl:value-of select="util:title('title', concat('Error details', $counter), concat('Error details', $counter), $ind1, false(), $vertical-orientation, false())"/>
		<xsl:value-of select="util:elements($ind1)"/>
		...
		<xsl:value-of select="util:chooseAmongThree('HL7 Error Code', .//ERR.3.9, .//ERR.3.2, .//ERR.3.1, $ind1)"/>
		<xsl:value-of select="util:element('Severity', .//ERR.4, $ind1)"/>
		<xsl:value-of select="util:chooseAmongThree('Application Error Code', .//ERR.5.9, .//ERR.5.2, .//ERR.5.1, $ind1)"/>
	...
	<xsl:function xmlns:xalan="http://xml.apache.org/xslt" name="util:format-tel2">
		<xsl:param name="areaCode"/>
	...
	<xsl:function xmlns:xalan="http://xml.apache.org/xslt" name="util:format-address-multilines">
	...
	<xsl:function xmlns:xalan="http://xml.apache.org/xslt" name="util:chooseAmongThree">
		<xsl:param name="elementName"/>
				...
				<xsl:choose>
					<xsl:when test="contains($time, '+')">
						...
						<xsl:if test="string-length($dateNoOffset) &gt; 13">
							...
							<xsl:value-of select="concat($month, $day, $year, ' ', $time-format, ' ', $AM-PM, ' +', normalize-space(substring-after($time, '+')))"/>
						</xsl:if>
	...
	<xsl:function xmlns:xalan="http://xml.apache.org/xslt" name="util:begin-sub-table">
		<xsl:param name="ind"/>
		<xsl:choose>
			<xsl:when test="$generate-plain-html"> </xsl:when>
			<xsl:otherwise>
	...
	<xsl:function xmlns:xalan="http://xml.apache.org/xslt" name="util:title-no-tab">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<!-- [Caro] why call util:title in a function that is not supposed to produce tabs ? util:title DOES produce tabs under certain circumstances-->
			</xsl:otherwise>
		</xsl:choose>
	</xsl:function>
	...
	<xsl:function xmlns:xalan="http://xml.apache.org/xslt" name="util:valueset">
		...
		<xsl:if test="count($HL7Tables/Tables/TableDefinition[@Id = $tablename]/t[@c = $key]) = 0"> </xsl:if>
	...
	<xsl:variable xmlns:fo="http://www.w3.org/1999/XSL/Format" name="HL7Tables">
		<Tables xmlns="">
			...
			<TableDefinition Codesys="erxQuantityCode" Id="erxQuantityCode" Version="1.1">
				<t c="C48473" d="Ampule"/>
				<t c="C62412" d="Applicator"/>
				<t c="C78783" d="Applicatorful"/>
				<t c="C48474" d="Bag"/>
				<t c="C48475" d="Bar"/>
				<t c="C53495" d="Bead"/>
				<t c="C54564" d="Blister"/>
				<t c="C53498" d="Block"/>
				<t c="C48476" d="Bolus"/>
				<t c="C48477" d="Bottle"/>
				<t c="C48478" d="Box"/>
				...
				<t c="C54702" d="Carton"/>
				<t c="C48481" d="Cartridge"/>
				<t c="C62414" d="Case"/>
				<t c="C69093" d="Cassette"/>
				<t c="C48484" d="Container"/>
				<t c="C48489" d="Cylinder"/>
				<t c="C16830" d="Device"/>
				<t c="C48490" d="Disk"/>
				<t c="C62417" d="Dose Pack"/>
				<t c="C96265" d="Dual Pack"/>
				<t c="C64933" d="Each"/>
				<t c="C53499" d="Film"/>
				<t c="C48494" d="Fluid Ounce"/>
				<t c="C101680" d="French"/>
				<t c="C48580" d="Gallon"/>
				<t c="C48155" d="Gram"/>
				<t c="C69124" d="Gum"/>
				<t c="C48499" d="Implant"/>
				<t c="C48501" d="Inhalation"/>
				<t c="C62275" d="Inhaler"/>
				<t c="C62418" d="Inhaler Refill"/>
				<t c="C62276" d="Insert"/>
				<t c="C67283" d="Intravenous Bag"/>
				<t c="C28252" d="Kilogram"/>
				<t c="C48504" d="Kit"/>
				<t c="C120263" d="Lancet"/>
				<t c="C48505" d="Liter"/>
				<t c="C48506" d="Lozenge"/>
				<t c="C48491" d="Metric Drop"/>
				<t c="C48512" d="Milliequivalent"/>
				<t c="C28253" d="Milligram"/>
				<t c="C28254" d="Milliliter"/>
				<t c="C28251" d="Millimeter"/>
				<t c="C71204" d="Nebule"/>
				<t c="C100052" d="Needle Free Injection"/>
				<t c="C69086" d="Ocular System"/>
				<t c="C48519" d="Ounce"/>
				<t c="C48520" d="Package"/>
				<t c="C48521" d="Packet"/>
				<t c="C65032" d="Pad"/>
				<t c="C82484" d="Paper"/>
				<t c="C48524" d="Patch"/>
				<t c="C120216" d="Pen Needle"/>
				<t c="C48529" d="Pint"/>
				<t c="C48530" d="Pouch"/>
				<t c="C48531" d="Pound"/>
				<t c="C97717" d="Pre-filled Pen Syringe"/>
				<t c="C65060" d="Puff"/>
				<t c="C111984" d="Pump"/>
				<t c="C48534" d="Quart"/>
				<t c="C62609" d="Ring"/>
				<t c="C71324" d="Sachet"/>
				<t c="C48536" d="Scoopful"/>
				<t c="C53502" d="Sponge"/>
				<t c="C48537" d="Spray"/>
				<t c="C53503" d="Stick"/>
				<t c="C48538" d="Strip"/>
				<t c="C48539" d="Suppository"/>
				<t c="C53504" d="Swab"/>
				<t c="C48540" d="Syringe"/>
				<t c="C48541" d="Tablespoon"/>
				<t c="C48542" d="Tablet"/>
				<t c="C62421" d="Tabminder"/>
				<t c="C48543" d="Tampon"/>
				<t c="C48544" d="Teaspoon"/>
				<t c="C54704" d="Tray"/>
				<t c="C48548" d="Troche"/>
				<t c="C48549" d="Tube"/>
				<t c="C38046" d="Unspecified"/>
				<t c="C48551" d="Vial"/>
				<t c="C48552" d="Wafer"/>
			</TableDefinition>
		</Tables>
