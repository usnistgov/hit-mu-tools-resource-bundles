...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:util="http://hl7.nist.gov" xmlns="" xpath-default-namespace="urn:hl7-org:v2xml" exclude-result-prefixes="xs" version="2.0">
	...
	<xsl:template match="MFI">
		<xsl:element name="fieldset">
			<xsl:element name="legend">Master File Identification</xsl:element>
...
-->
							<xsl:call-template name="util:element">
								...
								<xsl:with-param name="value" select="concat(./OMC.2.1,' (',./OMC.2.3,')')"/>
							</xsl:call-template>
						</xsl:if>
					</xsl:for-each>
					<!-- reference range -->
				...
				<xsl:element name="tbody">
					<!-- PM1.1 Health Plan ID -->
						...
						<xsl:call-template name="util:element">
							...
							<xsl:with-param name="value" select="concat(.//PM1.1.1,' (',.//PM1.1.3,')')"/>
						</xsl:call-template>
...
<!-- plain-html : create a head with css and a body around the "main" template to make it a plain html -->
	...
	<xsl:template name="css">
		...
		.test-data-specs-main fieldset table thead {border: 1px groove;background:#446BEC;text-align:left;}			
		.test-data-specs-main fieldset table tbody tr td {text-align:left;}			
		.test-data-specs-main .tds_noData td {background:#B8B8B8;}			
		.test-data-specs-main .tds_childField {background:#B8B8B8;}			
