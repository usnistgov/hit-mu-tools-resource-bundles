...
<xsl:stylesheet version="2.0" xmlns:xs="http://www.w3.org/2001/XMLSchema"
	...
	<xsl:template name="headerTemp">
		...
		<xsl:choose>
			<xsl:when test="../../..[@id = 'MFN_M08']">
				<thead>
					<tr>
						<th colspan="{$colspan}">Atomic Test : <xsl:value-of select="OM1.2/OM1.2.2"
			...
			<xsl:when test="../../..[@id = 'MFN_M10']">
				<thead>
					<tr>
						<th colspan="{$colspan}">Panel : <xsl:value-of select="OM1.2/OM1.2.2"/></th>
	...
	<xsl:template name="Admin">
		...
		<!-- ask Caroline where is OMC segment  -->
		...
		<table>
			...
			<tbody>
				<xsl:if test="exists(OM1.2)">
					<tr>
						<th>OM1.2</th>
						<th>Producer's Service/Test/Observartion ID </th>
						<th/>
						<th/>
					</tr>
					<xsl:copy-of
                        select="util:ID-text-format('OM1.2.1', 'Identifier', OM1.2/OM1.2.1, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM1.2.2', 'Text', OM1.2/OM1.2.2, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM1.2.3', 'Name of Coding System', OM1.2/OM1.2.3, '20px')"
				...
				<xsl:if test="exists(OM1.5)">
					...
					<xsl:copy-of
                        select="util:ID-text-format('OM1.5.1', 'Identifier', OM1.5/OM1.5.1, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM1.5.2', 'Text', OM1.5/OM1.5.2, '20px')"/>
				</xsl:if>
				<xsl:if test="exists(OM1.7)">
					<tr>
						<th>OM1.7</th>
					...
					<xsl:copy-of
                        select="util:ID-text-format('OM1.7.1', 'Identifier', OM1.7/OM1.7.1, '20px')"/>
					<xsl:copy-of

                        select="util:ID-text-format('OM1.7.2', 'Text', OM1.7/OM1.7.2, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM1.7.3', 'Name of Coding System', OM1.7/OM1.7.3, '20px')"
                    />
				...
				<xsl:copy-of
                    ...
                    select="util:ID-text-format('OM1.9', 'Preferred Report Name for the Observation', OM1.9, '0px')"/>
				<xsl:copy-of
                    select="util:ID-text-format('OM1.10', 'Preferred Short Name on Mnemonic for Observation', OM1.10, '0px')"/>
				<xsl:copy-of
                    select="util:ID-text-format('OM1.11', 'Preferred Long Name for the Observation', OM1.11, '0px')"/>
				<xsl:for-each select="OM1.32">

					<xsl:copy-of
                        select="util:ID-text-format('OM1.32', 'Interpretation of Observations', ., '0px')"
                    />
				</xsl:for-each>
				<xsl:for-each select="OM1.33">
					...
					<xsl:copy-of
                        ...
                        select="util:ID-text-format('OM1.33.1', 'Identifier', OM1.33.1, '20px')"/>
					<xsl:copy-of select="util:ID-text-format('OM1.33.2', 'Text', OM1.33.2, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM1.33.3', 'Name of Coding System', OM1.33.3, '20px')"
                    />
				</xsl:for-each>
				<xsl:for-each select="OM1.34">
					<tr>
					...
					<xsl:copy-of
                        select="util:ID-text-format('OM1.34.1', 'Identifier', OM1.34.1, '20px')"/>
					<xsl:copy-of select="util:ID-text-format('OM1.34.2', 'Text', OM1.34.2, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM1.34.3', 'Name of Coding System', OM1.34.3, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM1.34.4', ' Alternate Identifier', OM1.34.4, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM1.34.5', 'Alternate Text', OM1.34.5, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM1.34.6', 'Name of Alternate  Coding System', OM1.34.6, '20px')"
                    />
				</xsl:for-each>
				...
				<xsl:for-each select="OM1.35">
					<xsl:copy-of
                        select="util:ID-text-format('OM1.35', 'Rules that Trigger Reflex Testing', ., '0px')"
                    />
				</xsl:for-each>
				<xsl:for-each select="OM1.37">

					<xsl:copy-of
                        select="util:ID-text-format('OM1.37', 'Patient Preparation', ., '0px')"/>
				</xsl:for-each>
				...
				<xsl:copy-of
                    select="util:ID-text-format('OM1.39', 'Factors that may Affect the Observation', OM1.39, '0px')"/>
				...
				<xsl:for-each select="OM1.40">
					<xsl:copy-of
                        ...
                        select="util:ID-text-format('OM1.40', 'Service/Test/Observation Performance Schedule', ., '0px')"
                    />
				</xsl:for-each>
				<xsl:copy-of select="util:ID-text-format('OM1.48', 'Exclusive Test', OM1.48, '0px')"/>
				<xsl:copy-of
                    select="util:ID-text-format('OM1.49', 'Diagnostic Service Sector ID', OM1.49, '0px')"/>
				<xsl:for-each select="OM1.52">
					<tr>
						<th>OM1.52</th>
					...
					<xsl:copy-of
                        ...
                        select="util:ID-text-format('OM1.52.1', 'Identifier', OM1.52.1, '20px')"/>
					<xsl:copy-of select="util:ID-text-format('OM1.52.2', 'Text', OM1.52.2, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM1.52.3', 'Name of Coding System', OM1.52.3, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM1.52.4', ' Alternate Identifier', OM1.52.4, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM1.52.5', 'Alternate Text', OM1.52.5, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM1.52.6', 'Name of Alternate  Coding System', OM1.52.6, '20px')"
                    />
				</xsl:for-each>
				<xsl:for-each select="OM1.53">
					<xsl:copy-of
                        select="util:ID-text-format('OM1.53', 'Prior Results Instructions', ., '0px')"
                    />
				</xsl:for-each>
				<xsl:for-each select="OM1.54">
					<xsl:copy-of
                        select="util:ID-text-format('OM1.54', 'Special Instructions', ., '0px')"/>
				</xsl:for-each>
				<xsl:for-each select="OM1.55">
					<xsl:copy-of
                        select="util:ID-text-format('OM1.55', 'Test Relationship Category', OM1.55.2, '0px')"
                    />
				</xsl:for-each>
				<xsl:for-each select="OM1.56">
					...
					<xsl:copy-of
                        select="util:ID-text-format('OM1.56.1', 'Identifier', OM1.56.1, '20px')"/>
					<xsl:copy-of select="util:ID-text-format('OM1.56.2', 'Text', OM1.56.2, '20px')"/>

					<xsl:copy-of
                        select="util:ID-text-format('OM1.56.3', 'Name of Coding System', OM1.56.3, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM1.56.4', ' Alternate Identifier', OM1.56.4, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM1.56.5', 'Alternate Text', OM1.56.5, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM1.56.6', ' Name of Alternate Coding System', OM1.56.6, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM1.56.9', 'Original Text', OM1.56.9, '20px')"
                    />
				</xsl:for-each>
				<xsl:for-each select="OM1.57">
					...
					<xsl:copy-of
                        select="util:ID-text-format('OM1.57.1', 'Quantity', OM1.57.1, '20px')"/>
					<tr>
					...
					<xsl:copy-of
                        select="util:ID-text-format('OM1.57.2.1', ' Identifier', OM1.57.2/OM1.57.2.1, '30px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM1.57.2.2', 'Text', OM1.57.2/OM1.57.2.2, '30px')"/>
				</xsl:for-each>
				<xsl:for-each select="OM1.58">
					<tr>
						<th>OM1.58</th>
					...
					<xsl:copy-of
                        select="util:ID-text-format('OM1.58.1', 'Identifier', OM1.58.1, '20px')"/>
					<xsl:copy-of select="util:ID-text-format('OM1.58.2', 'Text', OM1.58.2, '20px')"/>

					<xsl:copy-of
                        select="util:ID-text-format('OM1.58.3', 'Name of Coding System', OM1.58.3, '20px')"/>
				</xsl:for-each>
				<xsl:for-each select="OM1.59">
					<tr>
						<th>OM1.59</th>
					...
					<xsl:copy-of
                        select="util:ID-text-format('OM1.59.1', 'Low Value', OM1.59.1, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM1.59.2', 'High Value', OM1.59.2, '20px')"/>
				</xsl:for-each>
			</tbody>
		</table>
	</xsl:template>
		...
		<table>
			...
			<tbody>
				<xsl:if test="exists(OMC.4)">
					...
					<xsl:copy-of
                        select="util:ID-text-format('OMC.4.1', 'Identifier', OMC.4/OMC.4.1, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OMC.4.2', 'Text', OMC.4/OMC.4.2, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OMC.4.3', 'Name of Coding System', OMC.4/OMC.4.3, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OMC.4.4', ' Alternate Identifier', OMC.4/OMC.4.4, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OMC.4.5', 'Alternate Text', OMC.4/OMC.4.5, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OMC.4.6', 'Name of Alternate  Coding System', OMC.4/OMC.4.6, '20px')"
                    />
				</xsl:if>
				<xsl:for-each select="OMC.5">
					<tr>
					...
					<xsl:copy-of
                        select="util:ID-text-format('OMC.5.1', 'Identifier', OMC.5.1, '20px')"/>
					<xsl:copy-of select="util:ID-text-format('OMC.5.2', 'Text', OMC.5.2, '20px')"/>

					<xsl:copy-of
                        select="util:ID-text-format('OMC.5.3', 'Name of Coding System', OMC.5.3, '20px')"/>
				</xsl:for-each>
				<xsl:if test="exists(OMC.6)">
					...
					<xsl:copy-of
                        select="util:ID-text-format('OMC.6.1', 'Identifier', OMC.6/OMC.6.1, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OMC.6.2', 'Text', OMC.6/OMC.6.2, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OMC.6.3', 'Name of Coding System', OMC.6/OMC.6.3, '20px')"
                    />
				</xsl:if>
				<xsl:copy-of select="util:ID-text-format('OMC.7', 'Answer Required', OMC.7, '0px')"/>

				<xsl:copy-of select="util:ID-text-format('OMC.8', 'Hint/Help Text', OMC.8, '0px')"/>

				<xsl:copy-of select="util:ID-text-format('OMC.9', 'Type of Answer', OMC.9, '0px')"/>
				...
				<xsl:if test="exists(OMC.11)">
					...
					<xsl:copy-of
                        select="util:ID-text-format('OMC.11.1', 'Identifier', OMC.11/OMC.11.1, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OMC.11.2', 'Text', OMC.11/OMC.11.2, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OMC.11.3', 'Name of Coding System', OMC.11/OMC.11.3, '20px')"
                    />
				</xsl:if>
				<xsl:copy-of
                    select="util:ID-text-format('OMC.12', 'Character Limit', OMC.12, '0px')"/>
				<xsl:copy-of
                    select="util:ID-text-format('OMC.13', 'Number of Decimals', OMC.13, '0px')"/>
		...
		<table>
			...
			<tbody>
				<xsl:if test="exists(OM2.2)">
					...
					<xsl:copy-of
                        select="util:ID-text-format('OM2.2.1', 'Identifier', OM2.2/OMC.2.2.1, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM2.2.2', 'Text', OM2.2/OM2.2.2, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM2.2.3', 'Name of Coding System', OM2.2/OM2.2.3, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM2.2.4', 'Alternate Identifier', OM2.2/OM2.2.4, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM2.2.5', 'Alternate Text', OM2.2/OM2.2.5, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM2.2.6', 'Name of Alternate Coding System', OM2.2/OM2.2.6, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM2.2.9', 'Original Text', OM2.2/OM2.2.9, '20px')"
                    />
				</xsl:if>
				<xsl:for-each select="OM2.6">
					...
					<xsl:if test="exists(OM2.6.1)">
						...
						<xsl:copy-of
                            select="util:ID-text-format('OM2.6.1.1', 'Low Value', OM2.6.1/OM2.6.1.1, '20px')"/>
						<xsl:copy-of
                            select="util:ID-text-format('OM2.6.1.2', 'High Value', OM2.6.1/OM2.6.1.2, '20px')"
                        />
					</xsl:if>
					<xsl:if test="exists(OM2.6.2)">
						...
						<xsl:copy-of
                            select="util:ID-text-format('OM2.6.2.1', 'Identifier', OM2.6.2/OM2.6.2.1, '20px')"/>
						<xsl:copy-of
                            select="util:ID-text-format('OM2.6.2.2', 'Text', OM2.6.2/OM2.6.2.2, '20px')"/>
						<xsl:copy-of
                            select="util:ID-text-format('OM2.6.2.3', 'Name of Coding System', OM2.6.2/OM2.6.2.3, '20px')"
                        />
					</xsl:if>
					<xsl:if test="exists(OM2.6.6)">
						...
						<xsl:copy-of
                            select="util:ID-text-format('OM2.6.6.1', 'Identifier', OM2.6.6/OM2.6.6.1, '20px')"/>
						<xsl:copy-of
                            select="util:ID-text-format('OM2.6.6.2', 'Text', OM2.6.6/OM2.6.6.2, '20px')"/>
						<xsl:copy-of
                            select="util:ID-text-format('OM2.6.6.3', 'Name of Coding System', OM2.6.6/OM2.6.6.3, '20px')"
                        />
					</xsl:if>
				</xsl:for-each>
				<xsl:for-each select="OM2.7">
					...
					<xsl:if test="exists(OM2.7.1)">
						...
						<xsl:copy-of
                            select="util:ID-text-format('OM2.7.1.1', 'Low Value', OM2.7.1/OM2.7.1.1, '20px')"/>
						<xsl:copy-of
                            select="util:ID-text-format('OM2.7.1.2', 'High Value', OM2.7.1/OM2.7.1.2, '20px')"
                        />
					</xsl:if>
					<xsl:if test="exists(OM2.7.2)">
						...
						<xsl:copy-of
                            select="util:ID-text-format('OM2.7.2.1', 'Identifier', OM2.7.2/OM2.7.2.1, '20px')"/>
						<xsl:copy-of
                            select="util:ID-text-format('OM2.7.2.2', 'Text', OM2.7.2/OM2.7.2.2, '20px')"/>
						<xsl:copy-of
                            select="util:ID-text-format('OM2.7.2.3', 'Name of Coding System', OM2.7.2/OM2.7.2.3, '20px')"
                        />
					</xsl:if>
					<xsl:if test="exists(OM2.7.6)">
						...
						<xsl:copy-of
                            select="util:ID-text-format('OM2.7.6.1', 'Identifier', OM2.7.6/OM2.7.6.1, '20px')"/>
						<xsl:copy-of
                            select="util:ID-text-format('OM2.7.6.2', 'Text', OM2.7.6/OM2.7.6.2, '20px')"/>
						<xsl:copy-of
                            select="util:ID-text-format('OM2.7.6.3', 'Name of Coding System', OM2.7.6/OM2.7.6.3, '20px')"
                        />
					</xsl:if>
				</xsl:for-each>
				<xsl:for-each select="OM2.8">
					...
					<xsl:if test="exists(OM2.8.1)">
						...
						<xsl:copy-of
                            select="util:ID-text-format('OM2.8.1.1', 'Low Value', OM2.8.1/OM2.8.1.1, '20px')"/>
						<xsl:copy-of
                            select="util:ID-text-format('OM2.8.1.2', 'High Value', OM2.8.1/OM2.8.1.2, '20px')"
                        />
					</xsl:if>
					<xsl:if test="exists(OM2.8.2)">
						...
						<xsl:copy-of
                            select="util:ID-text-format('OM2.8.2.1', 'Identifier', OM2.8.2/OM2.8.2.1, '20px')"/>
						<xsl:copy-of
                            select="util:ID-text-format('OM2.8.2.2', 'Text', OM2.8.2/OM2.8.2.2, '20px')"/>
						<xsl:copy-of
                            select="util:ID-text-format('OM2.8.2.3', 'Name of Coding System', OM2.8.2/OM2.8.2.3, '20px')"
                        />
					</xsl:if>
					<xsl:if test="exists(OM2.8.6)">
						...
						<xsl:copy-of
                            select="util:ID-text-format('OM2.8.6.1', 'Identifier', OM2.8.6/OM2.8.6.1, '20px')"/>
						<xsl:copy-of
                            select="util:ID-text-format('OM2.8.6.2', 'Text', OM2.8.6/OM2.8.6.2, '20px')"/>
						<xsl:copy-of
                            select="util:ID-text-format('OM2.7.6.3', 'Name of Coding System', OM2.8.6/OM2.8.6.3, '20px')"
                        />
					</xsl:if>
				</xsl:for-each>
			</tbody>
		...
		<table>
			...
			<tbody>
				<xsl:for-each select="OM3.4">
					...
					<xsl:copy-of
                        select="util:ID-text-format('OM3.4.1', 'Identifier', OM3.4.1, '20px')"/>
					<xsl:copy-of select="util:ID-text-format('OM3.4.2', 'Text', OM3.4.2, '20px')"/>

					<xsl:copy-of
                        select="util:ID-text-format('OM3.4.3', 'Name of Coding System', OM3.4.3, '20px')"
				...
				<xsl:for-each select="OM3.5">
					...
					<xsl:copy-of
                        select="util:ID-text-format('OM3.5.1', 'Identifier', OM3.5.1, '20px')"/>
					<xsl:copy-of select="util:ID-text-format('OM3.5.2', 'Text', OM3.5.2, '20px')"/>
					...
					<xsl:copy-of
                        select="util:ID-text-format('OM3.5.3', 'Name of Coding System', OM3.5.3, '20px')"
                    />
				</xsl:for-each>
				...
				<xsl:copy-of select="util:ID-text-format('OM3.7', 'Value Type', OM3.7, '0px')"/>
		...
		<table>
			...
			<tbody>
				<xsl:for-each select="OM4.3">
					<xsl:variable name="pos" select="position()"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM4.3', 'Container Description', ., '0px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM4.4', 'Container Volume', ../OM4.4[$pos], '0px')"/>
					...
					<xsl:copy-of
                        select="util:ID-text-format('OM4.5.2', 'Text', ../OM4.5[$pos]/OM4.5.2, '0px')"
                    />
				</xsl:for-each>
				<xsl:if test="exists(OM4.6)">
					...
					<xsl:copy-of
                        ...
                        select="util:ID-text-format('OM4.6.1', 'Identifier', OM4.6/OM4.6.1, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM4.6.2', 'Text', OM4.6/OM4.6.2, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM4.6.3', 'Name of Coding System', OM4.6/OM4.6.3, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM4.6.4', 'Alternate Identifer', OM4.6/OM4.6.4, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM4.6.5', 'Alternate Text', OM4.6/OM4.6.5, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM4.6.6', 'Name of Alternate Coding System', OM4.6/OM4.6.6, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('OM4.6.9', 'Original Text', OM4.6/OM4.6.9, '20px')"
                    />
				</xsl:if>
				<xsl:if test="exists(OM4.7)">
					...
					<xsl:copy-of
                        select="util:ID-text-format('OM4.7.2', 'Text', OM4.7/OM4.7.2, '20px')"/>
				</xsl:if>
				<xsl:if test="exists(OM4.10)">
					...
					<xsl:copy-of
                        select="util:ID-text-format('OM4.10.1', 'Quantity', OM4.10/OM4.10.1, '20px')"/>
					<tr>
					...
					<xsl:copy-of
                        select="util:ID-text-format('OM4.10.2.2', 'Text', OM4.10/OM4.10.2/OM4.10.2.2, '20px')"
                    />
				</xsl:if>
			</tbody>
		</table>
		...
		<table>
			...
			<tbody>
				<xsl:for-each select="OM5.2">
					...
					<xsl:copy-of
                        select="util:ID-text-format('OM5.2.1', 'Identifier', OM5.2.1, '20px')"/>
					<xsl:copy-of select="util:ID-text-format('OM5.2.2', 'Text', OM5.2.2, '20px')"/>

					<xsl:copy-of
                        select="util:ID-text-format('OM5.2.3', 'Name of Coding System', OM5.2.3, '20px')"
                    />
				</xsl:for-each>
			</tbody>
		</table>
	...
	<xsl:template name="chargeDescription">
		<table>
			...
			<tbody>
				<xsl:copy-of select="util:ID-text-format('CDM.3', 'Identifier', CDM.3, '0px')"/>
				...
				<xsl:for-each select="CDM.7">
					...
					<xsl:copy-of
                        select="util:ID-text-format('CDM.7.1', 'Identifier', CDM.7.1, '20px')"/>
					<xsl:copy-of select="util:ID-text-format('CDM.7.2', 'Text', CDM.7.2, '20px')"/>
		...
		<table>
			...
			<tbody>
				<xsl:if test="exists(PM1.1)">
					...
					<xsl:copy-of
                        select="util:ID-text-format('PM1.1.1', 'Identifier', PM1.1/PM1.1.1, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('PM1.1.2', 'Text', PM1.1/PM1.1.2, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('PM1.1.3', 'Name of Coding System', PM1.1/PM1.1.3, '20px')"
                    />
				</xsl:if>
				<xsl:for-each select="PM1.2">
					...
					<xsl:copy-of
                        select="util:ID-text-format('PM1.2.1', 'ID Number', PM1.2.1, '20px')"/>
					<tr>
						<th>PM1.2.4</th>
					...
					<xsl:copy-of
                        select="util:ID-text-format('PM1.2.4.1', 'Namespace ID', PM1.2.4/PM1.2.4.1, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('PM1.2.4.2', 'Universal ID', PM1.2.4/PM1.2.4.2, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('PM1.2.4.3', 'Universal ID Type', PM1.2.4/PM1.2.4.3, '20px')"
                    />
				</xsl:for-each>
			</tbody>
		</table>
		...
		<table>
			...
			<tbody>
				<xsl:if test="exists(MCP.3)">
					...
					<xsl:copy-of
                        select="util:ID-text-format('MCP.3.1', 'Quantity', MCP.3/MCP.3.1, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('MCP.3.2', 'Denomination', MCP.3/MCP.3.2, '20px')"
                    />
				</xsl:if>
				<xsl:if test="exists(MCP.4)">
					...
					<xsl:copy-of
                        select="util:ID-text-format('MCP.4.1', 'Quantity', MCP.4/MCP.4.1, '20px')"/>
					<xsl:copy-of
                        select="util:ID-text-format('MCP.4.2', 'Denomination', MCP.4/MCP.4.2, '20px')"
                    />
				</xsl:if>
				<xsl:copy-of
                    select="util:ID-text-format('MCP.5', 'Reason for Universal Service Cost Range', MCP.5, '  0px')"
                />
			</tbody>
		</table>
	</xsl:template>
		...
		<html>
			<head>
				<style type="text/css">
                    ...
                    @media print{
                    div[id='jurorContainer'] fieldset {font-size:small; overflow: visible !important;}  
                    div[id='jurorContainer'] fieldset table  th { font-size:x-small; }
                    ...
                    * [type=text]{
                    ...
                    div[id='jurorContainer'] fieldset table thead {border: 1px groove;background:#446BEC;text-align:left; color:white;}
						...
						<fieldset>
							...
							<table id="orderablePanel">
								<thead>
									...
									<tr>
										<th colspan="3">Orderable Panels and /or Tests</th>
							...
							<br/>
							<xsl:for-each select="//OM1.12[. = 'Y']/..">
								<!-- [Caroline] I changed a little bit to use a variable to test for the presence of data -->
								<xsl:variable name="CPOE"><xsl:call-template name="CPOE"/></xsl:variable>
								...
								<xsl:if test="string-length($CPOE) != 0">
									<table>
										...
										<tbody>
											<!--  [Caroline] we could re-use the $CPOE variable here -->
									...
									<xsl:if test="OM1.12[. != 'Y']">
										...
										<xsl:if test="string-length($CPOE) !=0 ">                  
											<table class="hideTable">
												<thead>
													<tr id="header">
														<th colspan="2"> Panel Component Details:
									...
									<xsl:when test="../../..[@id = 'MFN_M08']">
										...
										<table>
											<thead>
												...
												<tr>
													<th colspan="2">Atomic Test : <xsl:value-of
											...
											<tbody>
												<xsl:if test="exists(following-sibling::OM4)">
													<xsl:choose>
														<xsl:when
                                                  test="exists(following-sibling::OM4/OM4.16)">
                                                  ...
                                                  />
														...
														<xsl:otherwise>
															<xsl:for-each select="following-sibling::OM4">
										...
										<xsl:if
											...
											<table>
												<thead>
													...
													<tr>
														<th colspan="2">Panel : <xsl:value-of
												...
												<tbody>
													<xsl:choose>
														<xsl:when
                                                  test="exists(following-sibling::MFN_M10.MF_BATTERY.BATTERY_DETAIL/OM4/OM4.16)">
															<xsl:call-template name="specimen-tab">
																<xsl:with-param name="specimen"
								...
								<xsl:for-each select="//OM1.2/OM1.2.1[. = $panel]/../..">
									<xsl:if test="OM1.12[. != 'Y']">
										...
										<table>
											<thead>
												<tr id="header">
													<th colspan="3">Panel Component :<xsl:value-of
                                ...
                                <tbody>
                                    ...
                                    <xsl:for-each select="//OM1.2/OM1.2.1[. = $panel]/../..">
                                        <xsl:if test="OM1.12[. != 'Y']">
                                            <table>
                                                <tr id="header">
                                                  <th colspan="3" bgcolor=""> Panel Component
						...
						<xsl:for-each-group select="//MFE/.." group-by="MFE/MFE.4/MFE.4.2">
							...
							<xsl:choose>
								...
								<xsl:when test="../..[@id = 'MFN_M10']/current-group()">
									...
									<xsl:if test="exists(MFN_M10.MF_BATTERY.BATTERY_DETAIL/OM5)">
										<xsl:for-each select="MFN_M10.MF_BATTERY.BATTERY_DETAIL/OM5">
											<xsl:call-template name="observation-batteries"/>
										</xsl:for-each>

										<br/>
									</xsl:if>
									<xsl:if test="exists(MFN_M10.MF_BATTERY.BATTERY_DETAIL/OM4)">
										<xsl:for-each select="MFN_M10.MF_BATTERY.BATTERY_DETAIL/OM4">
											<xsl:call-template name="specimen-incorp"/>
